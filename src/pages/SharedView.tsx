...
const SharedView = () => {
  ...
  if (loading || authLoading) {
    return (
      <div className="min-h-screen bg-sarvam-ambient flex items-center justify-center">
  ...
  if ((error && !isPrivacyError) || !share) {
    return (
      <div className="min-h-screen bg-sarvam-ambient flex items-center justify-center p-4">
        <div className="card-premium-light p-8 text-center max-w-md mx-auto">
          ...
          <h1 className="text-2xl font-bold font-serif-elegant text-slate-900 mb-2">Share Not Found</h1>
          <p className="text-sm text-muted-foreground mb-6">{error || "This link may be invalid or expired."}</p>
          <Link to="/">
            <Button variant="outline" className="gap-1.5 rounded-full border-slate-200 bg-white/50">
  ...
  return (
      <div className="min-h-screen bg-sarvam-ambient">
        ...
        {/* Top bar */}
        <header className="sticky top-0 z-40 bg-white/60 backdrop-blur-md border-b border-slate-100/80 h-14 flex items-center px-4 sm:px-8">
          ...
          <div className="ml-auto">
            <Link to="/auth">
              <Button variant="outline" size="sm" className="text-xs font-semibold gap-1.5 rounded-full border-slate-200 bg-white/50">
        ...
        <main className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
          {/* Share card */}
          <div className="card-premium-light overflow-hidden animate-fade-in">
            {/* Banner */}
            ...
            <div className="px-4 sm:px-6 pb-6 -mt-8 relative z-10">
              <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center border-4 border-white shadow-md mb-4 ${color}`}>
                <Icon className="w-6 h-6 sm:w-7 sm:h-7" />
              </div>

              <Badge variant="secondary" className="text-[10px] mb-2 rounded-full bg-slate-100 text-slate-700 hover:bg-slate-150 border-none">{share.share_type}</Badge>
              <h1 className="text-2xl sm:text-3xl font-bold font-serif-elegant text-slate-900 leading-tight">
                {share.custom_title || `Shared ${share.share_type}`}
              </h1>

              {share.custom_message && (
                <div className="mt-3 p-3 sm:p-4 rounded-2xl bg-slate-50/50 border border-slate-100">
          ...
          {isPrivacyError ? (
            <div className="card-premium-light p-6 sm:p-8 mt-4 text-center border-orange-100 bg-orange-50/20 animate-fade-in">
              <div className="w-14 h-14 rounded-2xl bg-orange-100/50 flex items-center justify-center mx-auto mb-4">
              ...
              {error === "auth_required_college" && (
                <>
                  ...
                  <Link to="/auth" state={{ from: window.location.pathname }}>
                    <Button className="gap-1.5 font-semibold h-11 px-5 rounded-full bg-slate-900 text-white hover:bg-slate-800">
              ...
              {error === "restricted_college" && (
                <>
                  ...
                  <Button variant="outline" onClick={() => supabase.auth.signOut()} className="gap-1.5 h-11 px-5 rounded-full border-slate-200">
            ...
            <>
              ...
              {share.custom_links && share.custom_links.length > 0 && (
                <div className="card-premium-light p-4 sm:p-6 mt-4 animate-fade-in" style={{ animationDelay: "0.1s" }}>
                  ...
                  <div className="space-y-2">
                    {share.custom_links.map((link: string, i: number) => (
                      <a
                        ...
                        className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/50 border border-slate-100 hover:border-primary/30 transition-colors group"
                      >
              ...
              {share.custom_content && (
                <div className="card-premium-light p-4 sm:p-6 mt-4 animate-fade-in" style={{ animationDelay: "0.12s" }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-7 px-2.5 text-xs gap-1 rounded-full border-slate-200"
                      onClick={async () => {
                  ...
                  <div className={`rounded-2xl p-4 overflow-x-auto border ${
                    share.content_type === "code"
                      ? "bg-slate-950 text-emerald-400 font-mono text-xs leading-relaxed border-slate-900"
                      : "bg-slate-50/50 text-foreground text-sm leading-relaxed whitespace-pre-wrap border-slate-100"
                  }`}>
              ...
              {refData && (
                <div className="card-premium-light p-4 sm:p-6 mt-4 animate-fade-in" style={{ animationDelay: "0.15s" }}>
                    ...
                    <div className="space-y-2">
                      ...
                      {refData.file_path && (
                        <Button
                          size="sm"
                          variant="outline"
                          className="gap-1.5 mt-2 rounded-full border-slate-200"
                          onClick={() => {
                  ...
                  {share.share_type === "project" && (
                    <div className="space-y-2">
                      ...
                      <Badge variant="secondary" className="text-[10px] rounded-full bg-slate-100 text-slate-700 hover:bg-slate-150 border-none">{refData.status?.toUpperCase()}</Badge>
                      {refData.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {refData.tags.map((tag: string) => (
                            <Badge key={tag} variant="outline" className="text-[10px] rounded-full border-slate-200">{tag}</Badge>
                          ))}
                        </div>
                      )}
                    </div>
                  ...
                  {share.share_type === "opportunity" && (
                    <div className="space-y-2">
                      ...
                      <Badge variant="secondary" className="text-[10px] rounded-full bg-slate-100 text-slate-700 hover:bg-slate-150 border-none">{refData.category?.toUpperCase()}</Badge>
                      {refData.deadline && (
          ...
          <div className="text-center mt-8 animate-fade-in" style={{ animationDelay: "0.2s" }}>
            ...
            <Link to="/auth">
              <Button className="gap-1.5 font-semibold rounded-full bg-slate-900 text-white hover:bg-slate-800 px-6 h-11">
